<template>
  <div>
    ...
    >
      ...
      <img
        v-if="!(imgmesslist.length==0)"
        :src="imgmesslist[imp]"
      >
      <span
        v-else
        style="margin-bottom: 50px;display: inline-block;"
      >无</span>
      <div
        class="pag"
        v-if="!(imgmesslist.length==0)"
...
export default {
  ...
  methods: {
    ...
    handleclose() {
      (this.dialogVisibla = false), (this.display = false);
      this.form.message = "";
    },
    handleima(val) {
      this.impage = val - 1;
    ...
    addurl(event) {
      ...
      axios
        .post(
          `http://8.138.58.77:8806/gdufe/image/admin/upload/${this.trid}/abc`,
          formData,
          {
    ...
    getlist() {
      getList(this.dataurl, this.PageData.page, this.PageData.limit)
        .then((data) => {
  ...
  mounted() {
    (this.guanliid = sessionStorage.getItem("id")), console.log("111");
    this.getlist();
